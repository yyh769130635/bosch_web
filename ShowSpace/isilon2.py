...
def main(request):
    ...
    return render(request, 'ShowSpace/isilon2.html', {'result': values2, 'data': c.render_embed()})
...
def get_details(request, name):
    ...
    c = (Pie(init_opts=opts.InitOpts(width="600px", height="350px"))
        ...
        .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%", color="#000000", font_size=13))
        .set_colors(
        ["#FF6060", "#FF6B23", "#FFF260", "#6DF260", "#99FFCC", "#3DC1F2", "#909"])
    )
    ...
    d = (Line(init_opts=opts.InitOpts(width="1200px", height="300px"))
        ...
        markpoint_opts=opts.MarkPointOpts(
            data=[opts.MarkPointItem(type_="max", name="max", symbol_size=80)]),
    )
    )
